...
class FGDLoss(nn.Module):
    ...
    def forward(self,
        ...
        preds_S_device = preds_S.device
        preds_T_device = preds_T.device
    ...
    def spatial_pool(self, x, in_type):
        ...
        if in_type == 0:
            self.conv_mask_s.to(x_device)
            context_mask = self.conv_mask_s(x)
        else:
            self.conv_mask_t.to(x_device)
    ...
    def get_rela_loss(self, preds_S, preds_T):
        ...
        self.channel_add_conv_s.to(context_s.device)
        channel_add_s = self.channel_add_conv_s(context_s)
        out_s = out_s + channel_add_s
        
        self.channel_add_conv_t.to(context_t.device)
    ...
    def last_zero_init(self, m):
        if isinstance(m, nn.Sequential):
            torch.nn.init.constant_(m[-1].weight, val=0)
        else:
            torch.nn.init.constant_(m.weight, val=0)
        
    def reset_parameters(self):
        torch.nn.init.kaiming_uniform_(self.conv_mask_s.weight, mode='fan_in', nonlinearity='relu')
        torch.nn.init.kaiming_uniform_(self.conv_mask_t.weight, mode='fan_in', nonlinearity='relu')
